...
PAD_TOKEN = 129
GAP_TOKEN = 130
BASE_VOCAB = 128          # tokens 0..127
FULL_VOCAB = BASE_VOCAB + 3  # includes BOS, PAD, GAP
...
def pretty_parse(x: torch.Tensor) -> str:
    return ''.join(safe_chr(int(c)) for c in x.cpu().tolist())

def _align_pair(a: torch.Tensor, b: torch.Tensor) -> Tuple[List[int], List[int]]:
...
def opt_align_xs_to_zs(x0: torch.Tensor, x1: torch.Tensor):
    pairs = [_align_pair(x0[b], x1[b]) for b in range(x0.size(0))]
...
def rm_gap_tokens(z: torch.Tensor):
    ...
    max_len = max(len(r) for r in filtered)
...
def rv_gap_tokens(x: torch.Tensor, z_gap: torch.Tensor, z_pad: torch.Tensor):
...
def make_ut_mask_from_z(z_t: torch.Tensor, z_1: torch.Tensor, vocab_size: int):
    pad_token = PAD_TOKEN
    gap_token = GAP_TOKEN
    batch, L = z_t.shape
