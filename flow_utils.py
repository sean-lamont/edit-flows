...
def _align_pair(seq_0: torch.Tensor, seq_1: torch.Tensor, gap_token) -> Tuple[List[int], List[int]]:
    """
...
def naive_align_xs_to_zs(x_0: torch.Tensor, x_1: torch.Tensor, gap_token) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def shifted_align_xs_to_zs(x_0: torch.Tensor, x_1: torch.Tensor, gap_token, pad_token) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def opt_align_xs_to_zs(x_0: torch.Tensor, x_1: torch.Tensor, gap_token) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Aligns x_0 and x_1 to the same length by using a dynamic programming approach
    to find the minimum edit distance alignment.
    """
    aligned_pairs = [_align_pair(x_0[b], x_1[b], gap_token=gap_token) for b in range(x_0.shape[0])]
...
def rm_gap_tokens(z: torch.Tensor, gap_token, pad_token) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
...
def rv_gap_tokens(x: torch.Tensor, z_gap_mask: torch.Tensor, z_pad_mask: torch.Tensor, gap_token, pad_token) -> torch.Tensor:
    """
...
def safe_chr(c: int, gap_token, pad_token, bos_token,  show_special_chars=False, compact=False, ) -> str:
    if c == gap_token:
