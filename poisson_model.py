...
class PoissonEditFlowsTransformer(nn.Module):
    """
    ...
    def __init__(
        ...
        super(PoissonEditFlowsTransformer, self).__init__()
        ...
        assert mask_token_id < vocab_size, "mask_token_id must be less than vocab_size"
        ...
        self.rates_out = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.SiLU(),
            nn.Linear(hidden_dim, 3),  # Output 3 rates (insert, substitute, delete) as in original Edit Flows
        )
    ...
    def forward(self, tokens: T["batch", "x_seq_len", "long"],
        ...
        if torch.isnan(rates).any() or torch.isnan(sub_probs).any():
